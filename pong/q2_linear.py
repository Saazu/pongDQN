...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        image_height = state_shape[0]
        image_width = state_shape[1]
        channels = state_shape[2]
        self.s = tf.placeholder(tf.uint8, shape = (None, image_height, image_width, channels * self.config.state_history))
        self.a = tf.placeholder(tf.int32, shape=(None))
        self.r = tf.placeholder(tf.float32, shape=(None))
        self.sp = tf.placeholder(tf.uint8, shape= (None, image_height, image_width, channels * self.config.state_history))
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        start = layers.xavier_initializer()
        flat = layers.flatten(state, scope=scope)
        out = layers.fully_connected(inputs=flat, num_outputs=num_actions, 
                                    activation_fn=None, weights_initializer=start, 
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        assign_list = []
        for target_q_var, q_var in zip(target_q, q):
            assign_list.append(tf.assign(target_q_var, q_var))
    ...
    def add_loss_op(self, q, target_q):
        ...
        mask = tf.cast(tf.logical_not(self.done_mask), tf.float32)
        q_samp = self.r + mask * (self.config.gamma * tf.reduce_max(target_q, axis=1))

        q_sa = tf.reduce_sum(tf.multiply(tf.one_hot(self.a, depth=num_actions), q), axis=1)
        loss_matrix = tf.square(q_samp - q_sa)
    ...
    def add_optimizer_op(self, scope):
        ...
        training = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=scope)
        gradients = optimizer.compute_gradients(self.loss, training)
        ...
        if self.config.grad_clip is True:
            for grad, var in gradients:
                ...
                clipped = tf.clip_by_norm(grad, self.config.clip_val)
                clipped_gradients.append(clipped)

        self.train_op = optimizer.apply_gradients(zip(clipped_gradients, training))
